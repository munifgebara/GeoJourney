...
def organize_photos(source_folder, output_folder):
    ...
    with exiftool.ExifTool() as et:
        for idx, file_path in enumerate(files, start=1):
            ...
            if not image_hash:
                print("  ⮑ [SKIPPED] Could not generate hash.")
                skipped_no_hash += 1
                continue

            if used_exif:
                target_folder = output / str(photo_date.year) / f"{photo_date.month:02d}" / f"{photo_date.day:02d}"
            else:
            ...
            if not target_file.exists():
                ...
                if used_exif:
                    ...
                    print(f"  ⮑ [MOVED - NO EXIF] {file_path} -> {target_file}")
            else:
                # Arquivo já existe, comparar tamanhos
                src_size = file_path.stat().st_size
                dst_size = target_file.stat().st_size
                if src_size > dst_size:
                    # Remove o destino menor e move o maior
                    target_file.unlink()
                    shutil.move(str(file_path), str(target_file))
                    replaced += 1
                    print(f"  ⮑ [REPLACED] {file_path} (larger) replaced and moved to: {target_file}")
                elif src_size < dst_size:
                    # Remove o origem menor
                    file_path.unlink()
                    removed_src += 1
                    print(f"  ⮑ [REMOVED] {file_path} (smaller) removed, kept destination: {target_file}")
                elif src_size == dst_size:
                    # Mesmo tamanho, remove origem
                    file_path.unlink()
                    removed_src += 1
                    print(f"  ⮑ [REMOVED] {file_path} (duplicate same size), kept destination: {target_file}")
                else:
                    # Caso raro: não foi possível comparar ou agir
...
organize_photos("/mnt/7937a629-8811-4af0-ae7a-097a78cc4d2c/bkpSansung/.tx", "/srv/i7/.txo")
